...
vector<ul> hash1, hash2, hash1ByLength, hash2ByLength;
vector<int> v;
string s;
int times;
...
void hashInit() {
	...
	for (int i = 0; i < int(s.length()); i++) {
		if (i == 0) {
			...
			hash1.push_back((hash1[i - 1] * 26 + (s[i] - 'a')) % (UINT32_MAX));
			hash2.push_back((hash2[i - 1] * 27 + (s[i] - 'a')) % (UINT32_MAX));
		}
	}
}
...
void hashByLengthInit(int length) {
	...
	hash2ByLength.push_back(hash2[length - 1]);
	for (int i = length; i < int(s.length()); i++) {
		hash1ByLength.push_back(hash1[i] - ((hash1[i - length] * int(pow(26, length)))));
		hash2ByLength.push_back(hash2[i] - ((hash2[i - length] * int(pow(27, length)))));
	}
}
...
int p1Check() {
	...
	for (int i = 0; i < int(hash1ByLength.size()); i++) {
		counter = 1;
		for (int j = 0; j < i; j++) {
			if (hash1ByLength[j] == hash1ByLength[i] && hash2ByLength[j] == hash2ByLength[i]) {
				counter++;
			}
		}
